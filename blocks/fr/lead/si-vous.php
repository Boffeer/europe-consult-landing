<section class="lead">
	<div class="lead-decor__top"></div>
	<div class="container lead__container">
		<h2 class="section__title lead__title">
			Si vous voyez ce site, vos concurrents le voient aussi.
		</h2>
		<p class="lead__desc section__text">
			Par conséquent, choisissez plutôt l'heure et la date de la consultation, et après cela, augmentez vos gains de 2-5-10 fois.
		</p>
		<p class="section__caption lead__caption">
			<span class="lead__caption-text">
				<!-- <span class="lead__caption-text--highlight section__text--color">
					L'enregistrement de la consultation
				</span> -->
				La consultation après 24 heures sera payante
			</span>
		</p>
		<a href="<?php echo $global_links['calendly']; ?>" class="button button--primary lead__book button--calendly">
			<span class="button__inner">sélectionner la date et l'heure</span>
		</a>
		<!-- <div class="lead-decor__bottom"></div> -->
	</div>
</section>
